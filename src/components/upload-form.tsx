...
const LONG_AUDIO_WARNING_THRESHOLD_SEC = 60 * 60; // kept for reference but not used
...
export function UploadForm() {
  ...
  async function handleFileChange(event: ChangeEvent<HTMLInputElement>) {
    ...
    void updateDuration(file, conversionId);
    ...
    try {
      ...
      const message = error instanceof Error ? error.message : "音声変換に失敗しました。";
      setStatus({ phase: "error", message });
    }
  }

  async function updateDuration(file: File, conversionId: number) {
  ...
  async function handleSubmit(event: FormEvent<HTMLFormElement>) {
    ...
    if (status.phase !== "ready") {
      setStatus({
        phase: "error",
        message: selectedFile ? "変換が完了してからアップロードしてください。" : "音声ファイルを選択してください。",
      });
      return;
    }
    ...
    try {
      ...
      const storagePath = buildUserProjectSourceStoragePath(user.id, projectId, uploadFile.name);
      const firstPathSegment = storagePath.split("/")[0];

      console.log({ bucket: AUDIO_BUCKET, storagePath, userId: user.id, firstPathSegment });
      ...
      const { error: uploadError } = await supabase.storage.from(AUDIO_BUCKET).upload(storagePath, uploadFile, {
        contentType,
        upsert: false,
      });
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-5">
      <div className="space-y-2">
        <label htmlFor="audio" className="block text-sm font-medium text-zinc-800">
          音声ファイル
        </label>
        <input
          id="audio"
        ...
        <p className="text-sm text-zinc-500">
          mp3, m4a, wav / 最大 1GB。アップロード前に m4a、mono、16kHz、32kbps へ変換します。
        </p>
      </div>
      ...
      >
        {isUploading ? "アップロード中..." : busy ? "変換中..." : "文字起こしジョブを作成"}
      </button>
    </form>
  );
}

function TranscodeSummary({ status, selectedFile }: { status: TranscodeStatus; selectedFile: File | null }) {
  ...
  if (status.phase === "ready") {
    ...
    return (
      <div aria-live="polite" className="rounded-md border border-emerald-200 bg-emerald-50 px-3 py-2 text-sm text-emerald-900">
        <p className="font-medium">変換後ファイルをアップロードします</p>
        <p className="mt-1">
          元サイズ {formatBytes(result.originalSize)} → 変換後 {formatBytes(result.file.size)}（{result.reductionPercent}% 削減）
        </p>
      </div>
    );
  }
  ...
  return selectedFile ? (
    <p aria-live="polite" className="text-sm text-zinc-600">選択中: {selectedFile.name}（{formatBytes(selectedFile.size)}）</p>
  ) : null;
}
...
async function transcodeAudio(file: File, onProgress: (progress: number) => void): Promise<TranscodeResult> {
  ...
  await Promise.all([ffmpeg.deleteFile(inputName).catch(() => null), ffmpeg.deleteFile(outputName).catch(() => null)]);
  ffmpeg.terminate();

  const outputBuffer = new ArrayBuffer(outputData.byteLength);
  new Uint8Array(outputBuffer).set(outputData);

  const transcodedFile = new File([outputBuffer], outputName, { type: "audio/mp4" });
  const reductionPercent = Math.max(0, Math.round((1 - transcodedFile.size / file.size) * 100));

  return { file: transcodedFile, originalSize: file.size, reductionPercent };
}
