...
未分類 tips を画面上で紐づけるため、同 migration で管理者用の `artists` テーブルも作成します。候補の表示名はこのテーブルから読み込みます。
...
- metadata が不足している決済は `artist_id = uncategorized` として保存します。
...
- 月次締めでは `artist_id = uncategorized` の投げ銭を支払タスクから除外します。先に紐づけてから締めます。
