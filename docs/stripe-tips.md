...
- 取り込み時は Checkout Session、PaymentIntent、Charge の metadata を参照し、`artist_id` / `event_id` / `tip_type` を拾います。
- metadata が不足している決済は `artist_id = uncategorized`、`event_id = uncategorized` として保存します。
- 未分類の投げ銭は管理画面で artist/event/tip_type を手動で紐づけられます。
- 対象月を選択して、月別投げ銭一覧を確認できます。
- アーティスト別集計では `paid` だけを支払対象額として見ます。`failed` / `refunded` は集計から除外されます。
- 月次締めでは `uncategorized` の投げ銭を支払タスクから除外します。先に紐づけてから締めます。
...
curl -X POST http://localhost:3000/api/tips/checkout \
  ...
  -d '{"artistId":"artist_001","eventId":"event_001","tipType":"tip","amount":1000,"currency":"jpy"}'
```
...
9. 未分類の artist_id / event_id を入力して「紐づけ」を実行します。
10. 「月次締め」を実行します。
